...
<body>
  <header>
    <div style="display: flex; align-items: center; gap: 12px;">
      ...
      <div>
        ...
        <p>J111 polar-based AWA for selected Tasman routes. Enter TWD and TWS, then Calculate OR select Load Latest Wind.</p>
      </div>
    </div>
  </header>
  <main>
    <section>
      ...
      <!-- ========= NEW: SUPABASE WIND FETCH ========= -->

      <button class="btn" id="fetchWindBtn" style="margin-top: 6px;">📡 Load Latest Wind</button>
      <p id="fetchStatus" class="muted" style="margin-top: 4px;"></p>
      <!-- ========================================== -->
    ...
    <section>
      ...
      <div class="grid">
		...
		<div class="card">
  <div class="muted">Tide (No.2 Pile)</div>
  <div id="tideSpeedDisplay" class="kpi">—</div>
  <div id="tideDirDisplay" class="pill" style="margin-top: 4px;">—</div>
</div>
      </div>
    ...
    <section>
      ...
      <div id="sailRecommendations"><div class="muted">Calculate to see sail recommendations</div></div>
    </section>

    <section>
      <h2>Course Feasibility</h2>
      <div id="feasibilityResults"><div class="muted">Calculate to see course analysis</div></div>
    </section>
  </main>
  ...
  <!-- Main application logic (unchanged, with enhanced rotation handler) -->
  <script>
    // ========================
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      // --- POLARS ---
      const polars = {
        6: { angles: [44.5, 52, 60, 70, 75, 80, 90, 110, 120, 135, 150, 165, 180], speeds: [5.15, 5.66, 5.99, 6.20, 6.24, 6.25, 6.49, 6.43, 6.19, 5.62, 4.37, 3.55, 3.29] },
        8: { angles: [41.1, 52, 60, 70, 75, 80, 90, 110, 120, 135, 150, 165, 180], speeds: [5.99, 6.77, 7.09, 7.27, 7.30, 7.29, 7.57, 7.63, 7.53, 6.85, 5.70, 4.69, 4.38] },
        10: { angles: [40.6, 52, 60, 70, 75, 80, 90, 110, 120, 135, 150, 165, 180], speeds: [6.64, 7.43, 7.74, 7.93, 7.95, 7.94, 7.92, 8.35, 8.41, 7.76, 6.78, 5.80, 5.45] },
        12: { angles: [39.0, 52, 60, 70, 75, 80, 90, 110, 120, 135, 150, 165, 180], speeds: [6.89, 7.70, 8.01, 8.31, 8.43, 8.47, 8.35, 8.86, 9.11, 8.43, 7.56, 6.71, 6.35] },
        14: { angles: [38.2, 52, 60, 70, 75, 80, 90, 110, 120, 135, 150, 165, 180], speeds: [7.06, 7.87, 8.18, 8.50, 8.65, 8.79, 8.87, 9.26, 9.49, 9.11, 8.14, 7.46, 7.15] },
        16: { angles: [37.5, 52, 60, 70, 75, 80, 90, 110, 120, 135, 150, 165, 180], speeds: [7.10, 7.98, 8.29, 8.63, 8.80, 8.96, 9.27, 9.57, 9.88, 9.95, 8.70, 8.04, 7.77] },
        20: { angles: [38.3, 52, 60, 70, 75, 80, 90, 110, 120, 135, 150, 165, 180], speeds: [7.10, 7.97, 8.33, 8.76, 8.97, 9.17, 9.60, 10.11, 10.68, 11.93, 10.10, 9.14, 8.82] }
      };
      ...
      const sailPolygons = {
        "C0": [
          { tws: 5, awa: 32 }, { tws: 8, awa: 40 }, { tws: 12, awa: 50 },
          { tws: 13.5, awa: 60 }, { tws: 15, awa: 65 }, { tws: 16, awa: 70 },
          { tws: 15, awa: 75 }, { tws: 12, awa: 70 }, { tws: 10, awa: 65 },
          { tws: 7, awa: 60 }, { tws: 6, awa: 50 }, { tws: 5, awa: 40 }
        ],
        "A1.5": [
          { tws: 8, awa: 61 }, { tws: 8, awa: 80 }, { tws: 8.5, awa: 90 },
          { tws: 9, awa: 100 }, { tws: 10, awa: 110 }, { tws: 11, awa: 120 },
          { tws: 12.5, awa: 130 }, { tws: 16.5, awa: 137 }, { tws: 17, awa: 130 },
          { tws: 17, awa: 90 }, { tws: 16, awa: 80 }, { tws: 13, awa: 70 }
        ],
        "A2": [
          { tws: 15, awa: 80 }, { tws: 14.5, awa: 83 }, { tws: 15, awa: 90 },
          { tws: 16, awa: 100 }, { tws: 16, awa: 110 }, { tws: 16.5, awa: 120 },
          { tws: 17, awa: 130 }, { tws: 17.5, awa: 140 }, { tws: 22.5, awa: 150 },
          { tws: 25, awa: 148 }, { tws: 25, awa: 140 }, { tws: 23.5, awa: 130 },
          { tws: 23, awa: 120 }, { tws: 23, awa: 110 }, { tws: 23, awa: 100 },
          { tws: 22, awa: 90 }, { tws: 20, awa: 86 }
        ],
        "A4": [
          { tws: 22, awa: 90 }, { tws: 21, awa: 100 }, { tws: 22, awa: 110 },
          { tws: 22.5, awa: 120 }, { tws: 22.5, awa: 130 }, { tws: 23, awa: 140 },
          { tws: 25, awa: 150 }, { tws: 30, awa: 157 }, { tws: 31, awa: 150 },
          { tws: 29.5, awa: 140 }, { tws: 28, awa: 130 }, { tws: 27.5, awa: 120 },
          { tws: 27.5, awa: 110 }, { tws: 27, awa: 100 }, { tws: 22.5, awa: 90 }
        ],
        "A5": [
          { tws: 17.5, awa: 63 }, { tws: 17.5, awa: 70 }, { tws: 18.5, awa: 80 },
          { tws: 22, awa: 90 }, { tws: 24.5, awa: 100 }, { tws: 27, awa: 110 },
          { tws: 27, awa: 120 }, { tws: 27.5, awa: 130 }, { tws: 28, awa: 140 },
          { tws: 30, awa: 143 }, { tws: 33, awa: 140 }, { tws: 33, awa: 130 },
          { tws: 32, awa: 120 }, { tws: 31, awa: 110 }, { tws: 30, awa: 100 },
          { tws: 28, awa: 90 }, { tws: 25, awa: 80 }, { tws: 22, awa: 70 }, { tws: 20, awa: 65 }
        ],
        "A6": [
          { tws: 26, awa: 90 }, { tws: 25, awa: 100 }, { tws: 26, awa: 110 },
          { tws: 26.5, awa: 120 }, { tws: 26.5, awa: 130 }, { tws: 27, awa: 140 },
          { tws: 27, awa: 150 }, { tws: 35, awa: 157 }, { tws: 36, awa: 150 },
          { tws: 34.5, awa: 140 }, { tws: 33, awa: 130 }, { tws: 32.5, awa: 120 },
          { tws: 32.5, awa: 110 }, { tws: 32, awa: 100 }, { tws: 27.5, awa: 90 }
        ]
      };

      // --- J SAIL RANGES ---
      const jSailRanges = {
        "J1": { tws: { min: 0, max: 18 }, twa: { min: 20, max: 60 }, optimalTws: 12, optimalTwa: 35, category: "upwind" },
        "J2": { tws: { min: 12, max: 35 }, twa: { min: 20, max: 50 }, optimalTws: 18, optimalTwa: 35, category: "upwind" },
        "J3": { tws: { min: 30, max: 50 }, twa: { min: 20, max: 50 }, optimalTws: 35, optimalTwa: 35, category: "upwind" }
      };
      ...
      function boatSpeed(tws, twaDeg) {
        ...
        return { speed: spd, brackets: { t0, t1, a0_t0, a1_t0, a0_t1, a1_t1 } };
      }
      ...
      function isPointInPolygonWithTolerance(point, polygon, tolerance = 1.5) {
        ...
        const offsets = [
          { tws: 0, awa: tolerance }, { tws: 0, awa: -tolerance },
          { tws: tolerance, awa: 0 }, { tws: -tolerance, awa: 0 }
        ];
        for (const offset of offsets) {
          const testPoint = { tws: point.tws + offset.tws, awa: point.awa + offset.awa };
      ...
      function recommendSailsSimple(tws, awa) {
        ...
        if (tws >= 5 && tws <= 16 && absAwa >= 32 && absAwa <= 75) {
          let isInC0 = false;
          if (tws <= 8 && absAwa >= 32 && absAwa <= 40 + (tws-5)*2.67) isInC0 = true;
          else if (tws <= 12 && absAwa >= 32 + (tws-5)*4.5 && absAwa <= 50 + (tws-8)*6.67) isInC0 = true;
          else if (tws <= 16 && absAwa >= 50 + (tws-12)*5 && absAwa <= 70 + (tws-12)*1.25) isInC0 = true;
          if (isInC0) {
            const score = 100 - Math.abs(absAwa - 52) * 2;
            recommendations.push({ type: "C0", match: score > 80 ? "optimal" : "good", score: Math.max(0, Math.min(100, score)), details: "Code 0 - light air reaching sail", category: "flying" });
          }
        }
        if (tws >= 8 && tws <= 17 && absAwa >= 61 && absAwa <= 137) {
          const score = 100 - Math.abs(absAwa - 100) * 1.5;
          recommendations.push({ type: "A1.5", match: score > 80 ? "optimal" : "good", score: Math.max(0, Math.min(100, score)), details: "Light air asymmetric", category: "flying" });
        }
        return recommendations;
      }

      function calculatePolygonScore(point, polygon) {
        const centroid = polygon.reduce((acc, p) => ({ tws: acc.tws + p.tws, awa: acc.awa + p.awa }), { tws: 0, awa: 0 });
        centroid.tws /= polygon.length;
        centroid.awa /= polygon.length;
        const distance = Math.sqrt(Math.pow(point.tws - centroid.tws, 2) + Math.pow(point.awa - centroid.awa, 2));
      ...
      function recommendSailsPolygon(tws, twaSigned, apparentWindAngle) {
        ...
        if (recommendations.length === 0) {
          ...
          return recommendSailsSimple(tws, apparentWindAngle);
        }
        return recommendations;
      }
      ...
      function checkCourseFeasibility(twa, aws, tws, vb, awa) {
      ...
      function suggestAlternativeCourses(twd, bearing, currentTWA, tws) {
        ...
        if (absTWA < sailingLimits.j111.minVmgtwa) {
          ...
          alternatives.push({ type: "tack", bearing: portTackBearing, speed: portSpeed, twa: optimalUpwindTWA, description: `Port tack to ${portTackBearing.toFixed(0)}° (${optimalUpwindTWA}° TWA)` });
          alternatives.push({ type: "tack", bearing: starboardTackBearing, speed: stbdSpeed, twa: optimalUpwindTWA, description: `Starboard tack to ${starboardTackBearing.toFixed(0)}° (${optimalUpwindTWA}° TWA)` });
        }
        if (absTWA > optimalDownwindTWA + 20) {
          ...
          alternatives.push({ type: "gybe", bearing: gybeBearing, speed: gybeSpeed, twa: optimalDownwindTWA, description: `Gybe to ${gybeBearing.toFixed(0)}° (${optimalDownwindTWA}° TWA)` });
        }
        return alternatives;
      }

      // =============================================
      // UI INITIALIZATION & CALCULATE BUTTON
      // =============================================
        ...
        setTimeout(function() {
          try {
            ...
            if (brackets.t0 === brackets.t1) {
              document.getElementById('dAngles').textContent = polars[brackets.t0].angles.join(", ");
            } else {
              document.getElementById('dAngles').textContent = 
                `${brackets.t0}kn: ${polars[brackets.t0].angles.join(", ")} | ${brackets.t1}kn: ${polars[brackets.t1].angles.join(", ")}`;
            }
                  ...
                  <div class="card ${borderClass}" style="margin-bottom: 8px;">
                    ...
                    <div style="margin-top: 4px;">
                      <div style="background: #374151; border-radius: 4px; height: 6px; overflow: hidden;">
                        <div style="background: ${scoreClass === 'success' ? '#10b981' : scoreClass === 'accent' ? '#3b82f6' : '#f59e0b'}; height: 100%; width: ${rec.score}%;"></div>
                      </div>
                    </div>
                  </div>
                `;
  ...
  <!-- ========== SUPABASE CLIENT & FETCH SCRIPT ========== -->
  <script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>
  <script>
    // ============================================================
    //  SUPABASE WIND FETCH – does not alter any existing code
    //  Replace the placeholders below with your actual credentials.
    // ============================================================
    (function() {
      const supabaseUrl = 'https://sffjznjsxcnhetmiipui.supabase.co';
      const supabaseKey = 'sb_publishable_-XGKiGcqbPJP7G-W_6VdlQ_ScHG37Pu';

      if (!supabaseUrl.includes('YOUR_PROJECT') && !supabaseKey.includes('YOUR_ANON')) {
        const supabase = window.supabase.createClient(supabaseUrl, supabaseKey);

        const fetchBtn = document.getElementById('fetchWindBtn');
        const locationSelect = document.getElementById('locationSelect');
        const statusEl = document.getElementById('fetchStatus');

        const locationMap = {
          main_wharf: { speed: 'main_wharf_wind', dir: 'main_wharf_dir' },
          no2_pile:   { speed: 'no2_pile_velocity', dir: 'no2_pile_dir' },
          fairway:    { speed: 'fairway_wind', dir: 'fairway_dir' }
        };

function dirToDegrees(dir) {
  if (!dir) return null;
  
  // 1️⃣ Try parsing as a number (e.g., "104" → 104)
  const num = parseFloat(dir);
  if (!isNaN(num) && num >= 0 && num <= 360) {
    return num;
  }

  // 2️⃣ Fallback: cardinal directions (N, NE, etc.)
  const map = {
  ...
  return map[trimmed] !== undefined ? map[trimmed] : null;
}
...
fetchBtn.addEventListener('click', async function() {
  ...
  try {
    ...
    // --- Wind (Fairway) ---
    const windSpeed = record.fairway_wind;
    const windDirText = record.fairway_dir;

    if (windSpeed === null || windSpeed === undefined) {
      statusEl.textContent = '⚠️ Wind speed not available for Fairway.';
      statusEl.style.color = '#f59e0b';
      return;
    }

    let windDirDeg = dirToDegrees(windDirText);
    if (windDirDeg === null) {
      statusEl.textContent = `⚠️ Wind direction "${windDirText}" not recognised, using 0°.`;
      statusEl.style.color = '#f59e0b';
      windDirDeg = 0;
    }

    document.getElementById('twd').value = windDirDeg;
    document.getElementById('tws').value = windSpeed;

    // --- Tide (No.2 Pile) ---
    const tideSpeed = record.no2_pile_velocity;
    const tideDirText = record.no2_pile_dir;

    const tideSpeedEl = document.getElementById('tideSpeedDisplay');
    const tideDirEl = document.getElementById('tideDirDisplay');

    if (tideSpeed !== null && tideSpeed !== undefined) {
      tideSpeedEl.textContent = `${tideSpeed.toFixed(1)} kn`;
      tideSpeedEl.style.color = '#38bdf8';
    } else {
      tideSpeedEl.textContent = '—';
      tideSpeedEl.style.color = '';
    }

    // Classify tide direction
    let tideDirValue = dirToDegrees(tideDirText);
    let tideStatus = '—';
    if (tideDirValue !== null && !isNaN(tideDirValue)) {
      tideStatus = tideDirValue <= 200 ? 'Incoming' : 'Outgoing';
    } else {
      tideStatus = 'unknown';
    }
    tideDirEl.textContent = tideStatus;
    tideDirEl.style.color = tideStatus === 'Incoming' ? '#10b981' : tideStatus === 'Outgoing' ? '#f59e0b' : '#9ca3af';

    // Trigger calculation
    document.getElementById('calc').click();

    statusEl.textContent = `✅ Wind: ${windSpeed} kn from ${windDirText} (${windDirDeg}°)  |  Tide: ${tideSpeed !== null ? tideSpeed.toFixed(1) : '—'} kn (${tideStatus})`;
    ...
    fetchBtn.disabled = false;
        ...
        console.warn('Supabase credentials not configured. Fetch wind will not work.');
        const statusEl = document.getElementById('fetchStatus');
        if (statusEl) {
          statusEl.textContent = '⚠️ Supabase not configured. Edit the script.';
          statusEl.style.color = '#f59e0b';
        }
      }
    })();
